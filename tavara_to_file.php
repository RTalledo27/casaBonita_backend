...
foreach ($results as $row) {
    ...
    if (stripos($row->name, 'tavara') !== false || stripos($row->name, 'tavar') !== false) {
...
if ($tavaraId) {
    ...
    foreach ($contracts as $c) {
        $isVigente = $c->status === 'vigente' ? 'V' : 'X';
    ...
    $output .= "Vigentes + Financiados (contados): $vigFinanced\n";
    $output .= "Excel dice: 14\n";
    $output .= "Diferencia: " . ($vigFinanced - 14) . " contratos extra\n";
}

file_put_contents(__DIR__ . '/tavara_analysis.txt', $output);
echo $output;
echo "\n✅ Reporte guardado en tavara_analysis.txt\n";
